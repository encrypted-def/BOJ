...
int main(void) {
	...
	vector<int> state(1000001);
	int val = 0;
	int st = get<0>(query[0]);
	int en = get<1>(query[0]);
	for (int i = st; i <= en; i++) {
		if (state[A[i]] == 0) val++;
		state[A[i]]++;
	}
	...
	rep(i, 1, M) {
		int next_st = get<0>(query[i]);
		int next_en = get<1>(query[i]);
		if (st > next_st) {
			for(; st!=next_st; st--) {
				if (state[A[st-1]] == 0) val++;
				state[A[st-1]]++;
			}
		}
		else if (st < next_st) {
