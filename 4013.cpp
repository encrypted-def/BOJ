...
int main(void) {
	...
	vi cost(N + 1);
	rep(i, 1, N + 1) sf1(cost[i]);
	int S, P;
	sf2(S, P);
	vi rest(P);
	rep(i, 0, P) sf1(rest[i]);
	scc.getSCC();
	scc.getDAG();
	...
	vi D(scc.m);
	rep(i, 1, N + 1)
		tot[scc.group[i]] += cost[i];
	D[scc.group[S]] = tot[scc.group[S]];
	qi Q;
	for (int i = scc.group[S]; i >= 0; i--) {
		for (auto next : scc.sccadj[i]) {
			D[next] = max(D[next], D[i] + tot[next]);
		}
	}
	
	int mx = -1;
	for (auto r : rest) {
		mx = max(mx, D[scc.group[r]]);
	}
	pf1l(mx);
}
