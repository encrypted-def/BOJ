...
int segment[4000005]; // 범위 내에서 light가 켜져있는 갯수
int lazy[4000005];

void update_lazy(int node, int node_st, int node_en) {
	segment[node] = (node_en-node_st+1)- segment[node];
...
// i-j에 toggle하는 상황
void update_range(int i, int j, int node, int node_st, int node_en) {
	if (lazy[node] != 0)
	...
	update_range(i, j, 2 * node, node_st, (node_st + node_en) / 2);
	update_range(i, j, 2 * node + 1, (node_st + node_en) / 2 + 1, node_en);
