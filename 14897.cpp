...
void mo_add(int x){ // do something
  if(occur[arr[x]] == 0) cnt++;
  occur[arr[x]]++;
}
void mo_erase(int x){ // do something
  if(occur[arr[x]] == 1) cnt--;
  occur[arr[x]]--;
}
ti3 query[1000005];
...
int main(void){
  ...
  rep(i,0,n) arr[i] = lower_bound(all(tmp),arr[i])-tmp.begin();
  int m;
  sf1(m);
  for(int i = 0; i < m; i++){
    int l,r;
