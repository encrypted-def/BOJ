...
int main(void) {
	...
	CHT cht1(false); // 왼쪽으로 이동하는 경우
	cht1.pushline(-1, 0);
	ll mn = INF_LL;
	rep(i, 2, N + 1) {
		ll val = cht1.findval(-A[i]) + S[i-1]-i*A[i];
		if (mn > val) mn = val;
		cht1.pushline(-i, -S[i - 1]);
	}
	CHT cht2(false); // 오른쪽으로 이동하는 경우
	cht2.pushline(N, -S[N]);
	for (int i = N - 1; i >= 1; i--) {
		ll val = cht2.findval(A[i]) + (S[i] - i*A[i]);
		if (mn > val) mn = val;
		cht2.pushline(i, -S[i]);
	}
	if (mn > 0) mn = 0;
	rep(i, 1, N + 1) mn += A[i] * i;
	pf1l(-mn);
}
